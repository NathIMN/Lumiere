...
const updateQuestionnaireAnswer = asyncWrapper(async (req, res, next) => {
  ...
  if (!questionId || (answer === undefined && !req.file)) {
    return next(createCustomError("Question ID and answer (or file) are required", 400));
  }
  ...
  if (req.file) {
    try {
      ...
      const folderPath = `claims/${claimId}/questionnaire/${currentDate.getFullYear()}/${currentDate.getMonth() + 1}`;
      ...
      const documentData = {
        ...
        metadata: {
          description: `Questionnaire file for question: ${questionId}`,
          tags: ['questionnaire', 'claim-support'],
          questionId: questionId
        }
      };

      uploadedDocument = await Document.create(documentData);
      processedAnswer = uploadedDocument._id.toString();
      
      // Add document to claim documents array if not already present
      if (!claim.documents.includes(uploadedDocument._id)) {
        claim.documents.push(uploadedDocument._id);
      }
    } catch (uploadError) {
      return next(createCustomError(`File upload failed: ${uploadError.message}`, 500));
    }
  }

  try {
    await claim.updateQuestionnaireAnswer(questionId, processedAnswer);
...
const submitClaim = asyncWrapper(async (req, res, next) => {
  ...
  if (req.files && req.files.length > 0) {
    try {
      const uploadPromises = req.files.map(async (file) => {
        const currentDate = new Date();
        const folderPath = `claims/${claimId}/submission/${currentDate.getFullYear()}/${currentDate.getMonth() + 1}`;
        ...
        const documentData = {
          ...
          uploadedByRole: req.user.role,
          metadata: {
            description: `Supporting document for claim submission`,
            tags: ['claim', 'submission', 'supporting-document']
          }
        };
      ...
      const newDocumentIds = uploadedDocuments.map(doc => doc._id);
      claim.documents = [...claim.documents, ...newDocumentIds];
    } catch (uploadError) {
      return next(createCustomError(`File upload failed: ${uploadError.message}`, 500));
    }
  }
  ...
  // Add existing documents if provided (by ID)
...
const uploadClaimDocument = asyncWrapper(async (req, res, next) => {
  const { id: claimId } = req.params;
  const { documentType = 'supporting_document', description } = req.body;
  const { userId } = req.user;
  ...
  if (
    ...
    return next(
      createCustomError("You don't have permission to upload documents for this claim", 403)
    );
  }

  // Check if claim allows document uploads (not finalized)
  if (['approved', 'rejected', 'cancelled'].includes(claim.claimStatus)) {
    return next(
      createCustomError("Cannot upload documents to a finalized claim", 400)
    );
  }
  ...
  try {
    // Upload to Azure
    const currentDate = new Date();
    const folderPath = `claims/${claimId}/documents/${currentDate.getFullYear()}/${currentDate.getMonth() + 1}`;
    const uploadResult = await azureBlobService.uploadFile(req.file, folderPath);
    ...
    const documentData = {
      ...
      uploadedByRole: req.user.role,
      metadata: {
        description: description || `Document for claim ${claim.claimId}`,
        tags: ['claim', 'supporting-document'],
        claimId: claim.claimId,
        claimType: claim.claimType
      }
    };
    ...
    if (!claim.documents.includes(document._id)) {
