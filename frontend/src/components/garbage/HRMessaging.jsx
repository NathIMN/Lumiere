...
const HRMessaging = () => {
  ...
  // Use the messaging hook directly (without activeConversation dependency to avoid reconnections)
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (message) => {
      console.log('📨 NEW MESSAGE EVENT RECEIVED:', {
        ...
        willAdd: activeConversation?._id === message.conversationId
      });
      ...
      if (activeConversation && message.conversationId === activeConversation._id) {
        ...
        console.log('⏭️ Message not for current conversation, skipping UI update');
      }
    ...
    try {
      ...
      if (response.success) {
        ...
        const transformedConversations = response.conversations.map(conv => ({
          ...
          otherParticipant: conv.otherUser, // Map otherUser to otherParticipant
  ...
  const sendMessage = async () => {
    ...
    const recipientId = activeConversation.otherParticipant._id.toString();
    const sortedIds = [currentUserId, recipientId].sort();
    const conversationId = `conv_${sortedIds[0]}_${sortedIds[1]}`;
    
    // Send message via socket instead of HTTP
  ...
  const startConversation = async (contact) => {
    try {
      // Get conversation with the contact using the backend's endpoint
      const response = await messagingApiService.request(`/messages/conversation/${contact._id}`);
      if (response.success) {
        // Create a conversation object from the response - use backend's conversation ID format
        const currentUserId = (currentUser._id || currentUser.id).toString();
        ...
        if (socket) {
          console.log('🏠 Joining conversation room for:', contact._id);
      ...
      // If conversation doesn't exist yet, create a new empty conversation
      const currentUserId = (currentUser._id || currentUser.id).toString();
      const contactId = contact._id.toString();
      const sortedIds = [currentUserId, contactId].sort();
      const conversationId = `conv_${sortedIds[0]}_${sortedIds[1]}`;
  ...
  // Role filter options for HR Officer
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-gray-800">
              {messageLoading ? (
                ...
                messages.map((message, index) => {
                  // Fix user ID comparison - handle both _id and id formats from auth context
